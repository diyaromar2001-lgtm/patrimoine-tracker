...
import { X, Eye, RefreshCw, ChevronDown, ChevronUp } from "lucide-react"
...
export default function WatchlistPage() {
  ...
  function handleRemove(id: string) {
    ...
    if (items.find(i => i.id === id)?.ticker === selectedTicker) {
      setSelectedTicker(items[1]?.ticker ?? null)
      setSelectedName(items[1]?.name ?? "")
    }
  }

  return (
    <div className="flex flex-col">
      <Topbar title="Watchlist" subtitle={items.length + " actifs suivis" + (lastUpdated ? " · Mis à jour " + lastUpdated.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "")} />
      <div className="flex-1 space-y-6 p-6">
        ...
        <div className="grid gap-6 lg:grid-cols-5">
            ...
            <div className="space-y-1">
                ...
                return (
                  <motion.div key={item.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                    <button
                      ...
                      <div className="h-9 w-9 flex-shrink-0 rounded-lg flex items-center justify-center text-[11px] font-bold"
                      ...
                      <Sparkline data={item.sparkline.length ? item.sparkline : [price * 0.98, price]} positive={pct >= 0} width={52} height={24} />
              ...
              {filtered.length === 0 && (
                <div className="flex flex-col items-center py-10 gap-2">
                  ...
                  <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Recherchez un actif pour l'ajouter</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-3">
            {selectedTicker ? (
              <LiveChart ticker={selectedTicker} name={selectedName} height={380} showCompareSP500={true} />
            ) : (
              <div className="flex flex-col items-center justify-center h-64 rounded-xl border" style={{ backgroundColor: "var(--background-card)", borderColor: "var(--border)" }}>
                <Eye className="h-8 w-8 mb-2" style={{ color: "var(--foreground-dim)" }} />
                <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Sélectionnez un actif</p>
              </div>
            )}
          </div>
        </div>
